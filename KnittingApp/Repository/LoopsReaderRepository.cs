...
namespace KnittingApp.Repository
{
    public class LoopsReaderRepository:ILoopsReaderRepository
    {
        ...
        {
            var reader=await _context.LoopReaders.Where(r=>r.LoopReaderId==id).FirstOrDefaultAsync();
            if (reader == null)
                throw new ArgumentException($"LoopsReader с id {id} не существует");
            var loopMap=await _context.LoopMaps.Where(l=>l.LoopMapId==reader.LoopMapId).FirstOrDefaultAsync();
            if (loopMap == null)
                throw new ArgumentException($"LoopsMap для LoopReader с id {id} не существует");
            reader.LoopMap = loopMap.ToObject();
            return reader;
        }
        ...
        {
            var reader=await _context.LoopReaders.Where(r=>r.LoopMapId==loopMapId).FirstOrDefaultAsync();
            if(reader == null)
        ...
        {
            var readers= await _context.LoopReaders.Where(r=>r.UserId==userId).ToListAsync();
            return readers;
        }
